...
void Server::acceptConnections(fd_set *readFdsReady)
{
        ...
        it != _listenSockets.end(); ++it)
    {
        ...
        {
            std::ostringstream oss;
			oss << "DEBUG: Listen socket " << listenFd << " is ready for accepting" 
                << std::endl;
            LOG_ERROR(g_logger, oss.str());
...
void	Server::handleRequests(fd_set *readFdsReady)
{
		...
		it != _clientSockets.end(); ++it)
	{
		...
		{
		    std::ostringstream oss;
		    oss << "Client socket " << clientFd << " is ready for reading" << std::endl;
            LOG_DEBUG(g_logger, oss.str());
			...
			try
			{
				...
				{
				    std::ostringstream oss;
                    oss << "Creating new request for fd " << clientFd << std::endl;
                    LOG_DEBUG(g_logger, oss.str());
				...
				std::ostringstream oss;
                oss << "Request read returned: " << (requestComplete ? "COMPLETE" : "INCOMPLETE") << std::endl;
                LOG_DEBUG(g_logger, oss.str());
				...
				if (requestComplete)
				{
                    ...
                    oss1 << "Processing request and generating response" << std::endl;
                    LOG_DEBUG(g_logger, oss.str());
					...
					oss2 << "Switching socket " << clientFd << " to write mode" << std::endl;
                    LOG_DEBUG(g_logger, oss.str());
				...
				else
				{
                    ...
                    std::ostringstream oss2;
					oss2 << "Connection closed or error on fd " << clientFd << ", marking for removal" << std::endl;
                    LOG_DEBUG(g_logger, oss.str());
			...
			{
				std::ostringstream oss;
				oss << "Error handling request on fd " << clientFd << ": " << e.what() << std::endl;
                LOG_ERROR(g_logger, oss.str());
...
void Server::sendResponses(fd_set *writeFdsReady)
{
        ...
        it != _responses.end(); ++it)
    {
        ...
        if (clientFd < 0 || clientFd >= FD_SETSIZE) {
            ...
            std::ostringstream oss;
            oss << "Invalid file descriptor " << clientFd << std::endl;
            LOG_ERROR(g_logger, oss.str());
        ...
        oss << "Checking if socket " << clientFd << " is ready for writing: " 
        ...
        if (FD_ISSET(clientFd, writeFdsReady))
        {
            try
            {
                ...
                if (_clientSockets.find(clientFd) == _clientSockets.end()) {
                    ...
                    std::ostringstream oss;
                    oss << "Client socket not found for fd " << clientFd << std::endl;
                    LOG_ERROR(g_logger, oss.str());
                ...
                std::ostringstream oss;
                oss << "Attempting to send response on fd " << clientFd << std::endl;
                LOG_DEBUG(g_logger, oss.str());
                ...
                if (response.send(_clientSockets[clientFd]))
                {
                    ...
                    std::ostringstream oss;
                    oss << "Response fully sent on fd " << clientFd << std::endl;
                    LOG_DEBUG(g_logger, oss.str());
                    ...
                    if (response.shouldKeepAlive())
                    {
                        ...
                        std::ostringstream oss;
                        oss << "Marking connection for keep-alive: " << clientFd << std::endl;
                        LOG_DEBUG(g_logger, oss.str());
                ...
                {
                    std::ostringstream oss;
                    oss << "Response not fully sent yet, " << "will try again later" << std::endl;
                    LOG_DEBUG(g_logger, oss.str());
                }
            }
            catch (const std::exception& e)
            {
                std::ostringstream oss;
                oss << "Sending response on fd " << clientFd ":" << e.what() << std::endl;
                LOG_ERROR(g_logger, oss.str());
         ...
         it != toKeepAlive.end(); ++it)
    {
        ...
        std::ostringstream oss;
        oss << "Keeping connection alive, " << "switching " << clientFd << " back to read mode" << std::endl;
        LOG_DEBUG(g_logger, oss.str());
...
void	Server::run(void)
{
    ...
    std::ostringstream oss;
    oss << "select() checking range 0-" << _maxFd << " (" << _maxFd + 1 << " total), actually monitoring " 
    ...
    if (activity < 0)
    {
        if (errno != EINTR) { // Ignore if interrupted by signal
            std::ostringstream oss;
            oss << "Select: " << strerror(errno) << std::endl;
            LOG_ERROR(g_logger, oss.str());
        }
        return;
    }
    ...
    std::ostringstream oss1;
    oss1 << "Select() returned " << activity << " ready file descriptors" << std::endl;
    LOG_DEBUG(g_logger, oss.str());
