...
/*   Updated: 2025/07/26 17:44:47 by asanni           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	HttpRequest::read(Socket& clientSocket)
{
	...
	{
	    std::ostringstream oss1;
		std::cout << "Request parse state: " << _state << std::endl;
		_logger.log(LOG_DEBUG, oss1.str());
		switch (_state)
		{
			case REQUEST_LINE:
...
bool	HttpRequest::parseRequestLine(void)
{
	...
	if (endPos == std::string::npos)
	{
		...
		std::cout << "Request line incomplete, waiting for more data" << std::endl;
...
bool	HttpRequest::parseHeaders(void)
{
	while (true)
	{
		...
		if (endPos == std::string::npos)
		{
			...
			oss14 << "Headers incomplete, waiting for more data" << std::endl;
			_logger.log(LOG_DEBUG, oss14.str());
			return false;
		}
		...
		if (endPos == 0)
		{
			...
			oss15 << "End of headers found" << std::endl;
			_logger.log(LOG_DEBUG, oss15.str());
			...
			if (transferEncoding.find("chunked") != std::string::npos)
			{
				...
				oss16 << "Found chunked encoding" << std::endl;
				_logger.log(LOG_DEBUG, oss16.str());
				_chunked = true;
				_state = CHUNKED_SIZE;
			}
			else
			{
				...
				if (!contentLengthStr.empty())
				{
					...
					oss17 << "Content-Length: " << _contentLength << std::endl;
					_logger.log(LOG_DEBUG, oss17.str());
					_state = BODY;
				}
				else
				{
